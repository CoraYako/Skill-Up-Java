...
class TransactionControllerTest {
    ...
    void moneySendInPesos() {
        ...
        Mockito.when(userService.getByUserId(1L)).thenReturn(Optional.of(user1));

        Mockito.when(userService.getByUserId(2L)).thenReturn(Optional.of(user2));
        ...
        ResponseEntity<String> response = controller.moneySendInPesos(1L, 1000D, "INCOME", token);

        assertEquals(HttpStatus.ACCEPTED, response.getStatusCode());
        assertEquals(new ResponseEntity<>("Operación realizada exitosamente", HttpStatus.ACCEPTED), response);
    }
    ...
    void moneySendInUsd() {
        ...
        ResponseEntity<String> response = controller.moneySendInUsd(1L, 1000D, "INCOME", token);
